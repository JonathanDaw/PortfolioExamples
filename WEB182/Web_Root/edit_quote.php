...
if (!is_administrator()) {
	print '<h2>Acess Denied!</h2><p class="error">You do not have permission to access this page.</p>';
...
if (isset($_GET['id']) && is_numeric($_GET['id']) && ($_GET['id'] > 0)) { // Display the entry in a form:
	...
	if ($result = mysqli_query($dbc, $query)) { // Run the query.
		...
		<p><label>Quote <textarea name="quote" rows="5" cols="30">' . htmlentities($row['quote']) 
		. '</textarea></label></p>
	...
	if (!$problem) {
		...
		if ($result = mysqli_query($dbc, $query)) {
			...
			print '<p class="error">Could not update the quotation because:<br>' . mysqli_error($dbc) 
